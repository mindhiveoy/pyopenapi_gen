...
def test_sanitize_module_name_filter() -> None:
...
def test_sanitize_class_name_filter() -> None:
    renderer = TemplateRenderer()
...
def test_render_path_filter() -> None:
    renderer = TemplateRenderer()
    ...
    output = renderer.render("{{ render_path('/users/{userId}/items/{itemId}', {'userId': 42, 'itemId': 'abc'}) }}")
    assert output == "/users/42/items/abc"


def test_kwargsbuilder_global() -> None:
    renderer = TemplateRenderer()
    # KwargsBuilder is available as a global; build params and json
    tpl = "{% set kb = KwargsBuilder().with_params(a=1,b=None).with_json({'x': 2}) %}" "{{ kb.build() }}"
