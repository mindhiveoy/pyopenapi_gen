...
class TestModelVisitor(unittest.TestCase):  # Inherit from unittest.TestCase
    ...
    def test_visit_IRSchema_for_AgentDataSource_with_properties_generates_correct_fields(self) -> None:
        ...
        for imp_statement in generated_imports:
            if "import DataSource" in imp_statement and imp_statement.startswith("from ."):
                ...
                break
        self.assertTrue(
            found_data_source_import,  # Use self.assertTrue
    ...
    def test_visit_IRSchema_for_array_of_anonymous_objects_as_dataclass(self) -> None:
        ...
        # Expecting: items: List[MyListWrapperItem] or Optional[List[MyListWrapperItem]]
        # The name "MyListWrapperItem" would be generated by TypeHelper for the anonymous item schema.
        # DataclassGenerator makes this field Optional with default_factory=list
        # The generated code formats the field across multiple lines.
        expected_field_declaration_part1 = "items: Optional[List[MyListWrapperItem]] = field("
        ...
        # Search for the default_factory part, allowing for it to be on a new line and indented.
        # A simple string search might be brittle. Let's check for the key components.
        self.assertIn("MyListWrapperItem", generated_code, f"MyListWrapperItem type not found. Got:\n{generated_code}")
        self.assertIn(
            "default_factory=list",
            generated_code.replace("\n", "").replace(" ", ""),
            f"default_factory=list not found. Got (normalized):\\n{generated_code.replace(chr(10), ' ').replace(' ', '')}",
        )

        # A more robust check for the full structure if the simple assertIn fails due to formatting:
        # This regex looks for "items: Optional[List[MyListWrapperItem]] = field(" potentially followed by newlines/whitespace,
        # then "default_factory=list", then potentially more newlines/whitespace and a closing ")".
        import re

        # Define pattern string parts to avoid linter confusion
        part1 = "items:\\s*Optional\\[List\\[MyListWrapperItem\\]\\]\\s*=\\s*field\\s*\\("
        part2 = "\\s*default_factory\\s*=\\s*list\\s*"
        part3 = "\\)"
        pattern_str = part1 + part2 + part3
        pattern = re.compile(pattern_str)
        self.assertTrue(pattern.search(generated_code), f"Complete field pattern not matched. Got: {generated_code}")

        # TypeHelper, when resolving the anonymous item for DataclassGenerator, should have:
        # 1. Called _promote_anonymous_object_schema_if_needed with the item schema and a base name
        # 2. Generated a name (e.g., MyListWrapperItem)
        # 3. Added an import for this generated name.
        # The generated name for an anonymous item usually takes the form ParentNameItem.
        # The import will be relative like ".my_list_wrapper_item"
        self.assertIn(
            ".my_list_wrapper_item",  # CHECKING FOR RELATIVE MODULE KEY
            context.import_collector.relative_imports,  # CHECKING IN relative_imports
            "Relative import for generated item type MyListWrapperItem expected as key in relative_imports",
        )
        self.assertIn(
            "MyListWrapperItem",
            context.import_collector.relative_imports.get(
                ".my_list_wrapper_item", set()
            ),  # CHECKING NAME IN THE SET FOR THE RELATIVE MODULE
            "MyListWrapperItem class import expected from relative module .my_list_wrapper_item",
        )
