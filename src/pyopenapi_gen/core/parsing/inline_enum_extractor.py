...
def _extract_enum_from_property_node(
    ...
    if parent_schema_name:
        ...
        if property_key.lower() in ["status", "state", "type", "category", "role", "permission", "level"]:
            ...
            parent_class_name_for_enum = "Resource"
...
def _process_standalone_inline_enum(
    ...
    if not schema_obj.name and schema_name:
        ...
        logger.info(f"STANDALONE_ENUM_PROCESS: Generated more meaningful name '{schema_obj.name}' for an unnamed standalone enum.")
    ...
    if (
        ...
        while candidate_name in context.parsed_schemas:
            ...
            counter += 1
        logger.warning(
            f"STANDALONE_ENUM_PROCESS: Name collision for '{schema_obj.name}'. "
            f"Renaming to '{candidate_name}'. This might indicate a duplicate enum definition."
        )
        schema_obj.name = candidate_name

    # Ensure it's in the context with its final name
    if schema_obj.name:
        context.parsed_schemas[schema_obj.name] = schema_obj
        logger.debug(f"STANDALONE_ENUM_PROCESS: Ensured standalone enum '{schema_obj.name}' is in parsed_schemas.")

    return schema_obj


# Placeholder for the second function that will process remaining enums from loader.py logic
# def _process_remaining_inline_enums_in_ir_schemas(context: ParsingContext, logger: logging.Logger) -> None:
#     pass
