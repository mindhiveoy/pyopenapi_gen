...
class OpenAPISchemaResolver(SchemaTypeResolver):
    ...
    def resolve_schema(
        ...
        if (schema_type and 
            hasattr(self.ref_resolver, 'schemas') and 
            hasattr(self.ref_resolver.schemas, '__contains__') and 
            schema_type in self.ref_resolver.schemas):
    ...
    def _resolve_reference(self, ref: str, context: TypeContext, required: bool, resolve_underlying: bool = False) -> ResolvedType:
    ...
    def _resolve_named_schema(self, schema: IRSchema, context: TypeContext, required: bool) -> ResolvedType:
        ...
        try:
            ...
            if (hasattr(context, 'render_context') and 
                hasattr(context.render_context, 'current_file') and
        ...
        if (current_file and 
            isinstance(current_file, str) and 
            current_file.endswith(f"{module_stem}.py")):
        ...
        if (hasattr(context, 'render_context') and 
            hasattr(context.render_context, 'calculate_relative_path_for_internal_module')):
            try:
                # Calculate relative path from current location to the target model module
                target_module_relative = f"models.{module_stem}"
                relative_path = context.render_context.calculate_relative_path_for_internal_module(target_module_relative)
                if relative_path:
    ...
    def _resolve_array(self, schema: IRSchema, context: TypeContext, required: bool, resolve_underlying: bool = False) -> ResolvedType:
        """Resolve array type."""
        ...
        items_resolve_underlying = (
            resolve_underlying and 
            items_schema.name and 
            not getattr(items_schema, 'properties', None) and 
            items_schema.type in ('string', 'integer', 'number', 'boolean')
        )
        item_type = self.resolve_schema(items_schema, context, required=True, resolve_underlying=items_resolve_underlying)
        context.add_import("typing", "List")
    ...
    def _resolve_any_of(self, schema: IRSchema, context: TypeContext, required: bool, resolve_underlying: bool = False) -> ResolvedType:
        ...
        for sub_schema in schema.any_of:
            ...
            sub_resolve_underlying = (
                resolve_underlying and 
                sub_schema.name and 
                not getattr(sub_schema, 'properties', None) and 
                sub_schema.type in ('string', 'integer', 'number', 'boolean')
            )
            sub_resolved = self.resolve_schema(sub_schema, context, required=True, resolve_underlying=sub_resolve_underlying)
    ...
    def _resolve_all_of(self, schema: IRSchema, context: TypeContext, required: bool, resolve_underlying: bool = False) -> ResolvedType:
    ...
    def _resolve_one_of(self, schema: IRSchema, context: TypeContext, required: bool, resolve_underlying: bool = False) -> ResolvedType:
