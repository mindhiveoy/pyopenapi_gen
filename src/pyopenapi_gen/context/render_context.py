...
class RenderContext:
    ...
    At render time, imports are emitted as relative or absolute depending on whether the target is a generated module or an external dependency.
    """
    ...
    def add_import(self, module: str, symbol: str) -> None:
        ...
        Parameters:
            module (str): The module path to import from. Must use dot notation (e.g., 'models.agent_history', 'endpoints.analytics').
        ...
        if module.startswith("models.") or module.startswith("endpoints."):
            ...
            if rel_module and rel_module.strip("."):
                with open("/tmp/pyopenapi_gen_import_debug.log", "a") as debug_log:
                    debug_log.write(
                        f"add_relative_import: rel_module={rel_module!r}, symbol={symbol!r}\n"
                    )
    ...
    def add_typing_imports_for_type(self, type_str: str) -> None:
        """
        Adds imports for all typing types present in a type string, e.g. Optional, List, Dict, Set, Tuple, Union, Any, AsyncIterator, etc.
        Uses regex to match both generic and non-generic usages, including Any inside generics.
        """
