...
class TypeHelper:
    ...
    def get_python_type_for_schema(
        ...
        if (
            ...
            if is_circular:
                # For circular references, use forward references with TYPE_CHECKING
                context.add_import("typing", "TYPE_CHECKING")

                # Add conditional import
                if hasattr(context, "add_conditional_import"):
                    context.add_conditional_import("TYPE_CHECKING", model_module_path, class_name_to_import)
                else:
                    # Add TYPE_CHECKING guard comment as a note
                    logger.debug(
                        f"Forward reference needed for {class_name_to_import} from {model_module_path} "
                        f"due to circular dependency. Add TYPE_CHECKING guard manually."
                    )
            else:
                # Normal case - direct import
                context.add_import(model_module_path, class_name_to_import)

            resolved_type_str = class_name_to_import
            # Finalize (e.g., add Optional) using the original schema's requirement status
            return TypeFinalizer(context, all_schemas).finalize(resolved_type_str, schema, required)

        # Default path: use SchemaTypeResolver for more complex cases or standard types
        # SchemaTypeResolver will internally call other specialized resolvers.
